...
class ServicAgg():
    def __init__(self, args, ServiceModel, client_gard):
        ...
        self.args = args
        self.shared_parameters = list((p for n, p in ServiceModel.named_parameters() if "task" not in n))
    ...
    def backward(self):
        optimizer = optim.SGD(self.ServiceModel.parameters(), lr=self.args.global_lr)
        ...
        overwrite_grad(self.shared_parameters, gt, grad_dims)
        optimizer.step()

        return dt, self.ServiceModel